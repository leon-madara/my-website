...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize scroll fade controller for header elements
    if (typeof initScrollFadeController !== 'undefined') {
        console.log('🔄 Initializing scroll fade controller for header elements...');
        try {
            initScrollFadeController({
                targets: ['.header-logo', '.nav-pills', '.theme-toggle'],
                fadeDelay: 1500, // 1.5 seconds delay
                fadeDuration: 300, // 300ms fade duration
                scrollThreshold: 10 // Minimum scroll distance to trigger
            });
            console.log('✓ Scroll fade controller initialized successfully');
        } catch (error) {
            console.warn('⚠️ Scroll fade controller initialization failed:', error);
        }
    } else {
        console.warn('⚠️ ScrollFadeController not found - scroll fade disabled');
    }
});
...
function initParallaxHero() {
    ...
    if (experienceSection) {
        ...
        experienceTimeline.fromTo(timelineItems,
            { opacity: 0, x: -60 },
            { opacity: 1, x: 0, duration: 1, stagger: 0.2, ease: 'power2.out', immediateRender: false },
            '-=0.5'
        );
    }
    ...
    if (certificationsSection) {
        ...
        // Animate certification cards - vertical entrance for stacked layout
        certificationsTimeline.fromTo(certCards,
            { opacity: 0, scale: 0.95, y: 40 },
            { opacity: 1, scale: 1, y: 0, duration: 1, stagger: 0.15, ease: 'power2.out' },
            '-=0.5'
        );
    }
...
function activateFallbackMode(reason) {
    ...
    // Other sections that may be left hidden if GSAP/ScrollTrigger fails
    const allSectionTitles = document.querySelectorAll('.section-title');
    const allSectionBodies = document.querySelectorAll('.section-body');
    const timelineItems = document.querySelectorAll('.timeline-item');
    ...
    // Ensure all section titles/bodies are visible (CSS sets some to opacity:0 for animation)
    ...
    allSectionBodies.forEach(body => {
        body.style.opacity = '1';
        body.style.transform = 'none';
        body.style.visibility = 'visible';
    });

    // Ensure Experience timeline items are visible
