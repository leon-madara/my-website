...
LeonPortfolio.EntranceAnimations = {
    ...
    setupFooterAnimation() {
        ...
        const applyAnimation = () => {
            ...
            // Remove inline styles to allow CSS class to take effect
            footer.style.opacity = '';
            footer.style.transform = '';
        };

        // Since footer is fixed at bottom, it's always in view - animate immediately
        // But use IntersectionObserver as a fallback for edge cases
        ...
        // Immediate trigger for fixed footer (always in view)
        // Use requestAnimationFrame to ensure DOM is ready
        requestAnimationFrame(() => {
            applyAnimation();
        });

        // Fallback: Ensure footer is visible after a short delay if observer doesn't trigger
        setTimeout(() => {
            if (!footer.classList.contains('animate-in')) {
                applyAnimation();
            }
        }, 500);
    },
    ...
    showAllElements() {
        ...
        if (footer) {
            footer.classList.add('animate-in');
            // Remove inline styles to allow CSS class to work
            footer.style.opacity = '';
            footer.style.transform = '';
        }
