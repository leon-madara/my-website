...
const LeonPortfolio = {
    ...
    initializeComponents() {
        ...
        // Initialize FloatingNavigation with halo system integration
        this.floatingNav = new this.FloatingNavigation();

        // Initialize navigation icon click handlers
        this.initializeNavigationIcons();
    ...
    initializeResponsiveFeatures() {
        ...
        if (this.floatingNav) {
            ...
            this.floatingNav.monitorPerformance();
        }
    ...
    initializeNavigationIcons() {
        ...
        console.log('Navigation icon click handlers initialized');
    },

    // Handle navigation icon click
    handleIconClick(event, iconElement) {
        ...
        const sectionId = iconElement.getAttribute('data-section');
        const currentActive = document.querySelector('.nav-icon.active');

        // If clicking the same icon, do nothing
        if (currentActive === iconElement) {
            return;
        }

        // Set transitioning state to prevent rapid clicks
        this.state.isTransitioning = true;

        // Remove active state from previous icon with smooth transition
        if (currentActive) {
        ...
        iconElement.setAttribute('aria-current', 'page');
        ...
        const label = iconElement.getAttribute('aria-label') || iconElement.getAttribute('data-label') || 'Navigation item';
...
// FloatingNavigation class for managing navigation interactions
LeonPortfolio.FloatingNavigation = class {
    constructor(containerSelector = '.floating-navigation') {
        this.container = LeonPortfolio.utils.getElement(containerSelector);
        this.navItems = [];
    ...
    init() {
        ...
        console.log('FloatingNavigation initialized');
    }

    createBackgroundBlur() {
        // Create blurry black background element
        this.backgroundBlur = LeonPortfolio.utils.createElement('div', {
            className: 'nav-background-blur',
            'aria-hidden': 'true'
        });

        // Get nav-container element
        const navContainer = this.container.querySelector('.nav-container');
        if (navContainer) {
            // Insert background blur into nav-container (behind icons)
            navContainer.insertBefore(this.backgroundBlur, navContainer.firstChild);

            // Calculate and set width based on nav-container content
            this.updateBackgroundBlurSize();
        }
    }

    updateBackgroundBlurSize() {
        if (this.backgroundBlur) {
            // Calculate width based on nav-container content
            const navContainer = this.container.querySelector('.nav-container');
            if (navContainer) {
                const containerRect = navContainer.getBoundingClientRect();
                const padding = 20; // Extra padding for visual effect
                this.backgroundBlur.style.width = `${containerRect.width + padding}px`;
            }
        }
    }

    createHalo() {
        // Create halo element for elevation effect
        this.halo = LeonPortfolio.utils.createElement('div', {
            className: 'nav-halo',
            'aria-hidden': 'true'
        });

        // Insert halo into navigation container
        this.container.appendChild(this.halo);

        // Initially hide the halo
        this.halo.style.opacity = '0';
        this.halo.style.transform = 'scale(0)';
        this.halo.style.transition = `all ${LeonPortfolio.config.haloAnimationDuration}ms ease`;
    }
    ...
    bindEvents() {
        this.navItems.forEach((item, index) => {
            ...
            item.addEventListener('touchend', (e) => this.handleTouchEnd(e, item), { passive: true });

            // Keyboard events
            item.addEventListener('keydown', (e) => this.handleKeyDown(e, item, index));
            item.addEventListener('focus', (e) => this.handleItemFocus(e, item));
            item.addEventListener('blur', (e) => this.handleItemBlur(e, item));
        });

        // Container leave event to hide halo
        this.container.addEventListener('mouseleave', () => this.dematerializeHalo());

        // Window resize event to update background blur size
    ...
    setupAccessibility() {
        ...
        this.navItems.forEach((item, index) => {
            ...
            item.setAttribute('aria-setsize', this.navItems.length);
        });
    }

    handleItemHover(event, item) {
        if (this.isAnimating || LeonPortfolio.state.isMobile) return;

        this.elevateIcon(item);
        this.materializeHalo(item);
    }

    handleItemLeave(event, item) {
        if (this.isAnimating || LeonPortfolio.state.isMobile) return;

        this.descendIcon(item);
    }
    ...
    handleItemClick(event, item) {
        ...
        const label = item.getAttribute('aria-label') || 'Navigation item';
        LeonPortfolio.utils.announceToScreenReader(`Navigated to ${label}`);
    }

    handleTouchStart(event, item) {
        if (!LeonPortfolio.state.isMobile) return;
    ...
    handleItemFocus(event, item) {
        // Show elevation on keyboard focus
        this.elevateIcon(item);
        this.materializeHalo(item);

        // Announce elevation state and tooltip content
        const elevationAnnouncer = document.getElementById('elevation-announcements');
        if (elevationAnnouncer) {
            const label = item.getAttribute('aria-label') || 'Navigation item';
            const tooltipText = item.getAttribute('data-label') || label;
            elevationAnnouncer.textContent = `${label} focused and elevated. Tooltip: ${tooltipText}`;
        }

        // Ensure tooltip is visible for keyboard users
        this.showTooltipForAccessibility(item);
    }

    handleItemBlur(event, item) {
        // Remove elevation on blur
        this.descendIcon(item);

        // Hide accessibility tooltip
        this.hideTooltipForAccessibility(item);

        // Hide halo if no other item is focused
        setTimeout(() => {
            if (!this.container.contains(document.activeElement)) {
                this.dematerializeHalo();
            }
        }, 10);
    }
    ...
    elevateIcon(item) {
        ...
        // Add transform for elevation effect
        const icon = item.querySelector('.nav-icon');
        if (icon) {
            icon.style.transform = 'translateY(-3px) scale(1.1)';
            icon.style.transition = `transform ${LeonPortfolio.config.animationDuration}ms ease`;
        }
    }
    ...
    materializeHalo(item) {
        if (!item) return;

        // Use the advanced halo system if available
        if (LeonPortfolio.haloSystem) {
            // Remove any existing halo for this item
            if (this.currentHaloId) {
                LeonPortfolio.haloSystem.dematerializeHalo(this.currentHaloId);
            }

            // Create and materialize new halo
            this.currentHaloId = LeonPortfolio.haloSystem.createAndMaterializeHalo(item, {
                size: 45,
                pulse: item.classList.contains('active'),
                duration: LeonPortfolio.config.haloAnimationDuration
            });
        } else {
            // Fallback to simple halo if advanced system not available
            if (!this.halo) return;
            ...
            // Position and show halo
            this.halo.style.left = `${left}px`;
            this.halo.style.top = `${top}px`;
            this.halo.style.opacity = '1';
            this.halo.style.transform = 'scale(1)';
        }
    }

    dematerializeHalo() {
        // Use the advanced halo system if available
        if (LeonPortfolio.haloSystem && this.currentHaloId) {
            LeonPortfolio.haloSystem.dematerializeHalo(this.currentHaloId);
            this.currentHaloId = null;
        } else {
            // Fallback to simple halo
            if (!this.halo) return;
    ...
    focusPreviousItem(currentIndex) {
        const previousIndex = currentIndex > 0 ? currentIndex - 1 : this.navItems.length - 1;
        this.navItems[previousIndex].focus();
    }

    focusNextItem(currentIndex) {
        const nextIndex = currentIndex < this.navItems.length - 1 ? currentIndex + 1 : 0;
    ...
    // Update navigation for mobile/desktop state - Requirements 8.1, 8.2, 8.3, 8.4, 8.5
    updateForMobile(isMobile) {
        ...
        // Ensure 60px header height is maintained - Requirement 8.1
        this.ensureHeaderHeight();

        // Ensure 30px icon height is maintained - Requirement 8.2
        this.ensureIconHeight();

        // Update container layout for viewport - Requirement 8.3
        this.updateContainerLayout();

        // Update touch interactions - Requirements 8.4, 8.5
        this.updateTouchInteractions(isMobile);
    }

    // Ensure 60px header height across all devices - Requirement 8.1
    ensureHeaderHeight() {
        const navigation = document.querySelector('.floating-navigation');
        if (navigation) {
            navigation.style.height = '60px';
    ...
    // Ensure 30px icon height across all devices - Requirement 8.2
    ensureIconHeight() {
        const icons = this.container.querySelectorAll('.nav-icon-svg');
        icons.forEach(icon => {
            icon.style.width = '30px';
            icon.style.height = '30px';
        });
    ...
    // Update container layout for different viewport widths - Requirement 8.3
    updateContainerLayout() {
        const viewportWidth = window.innerWidth;
        const iconsContainer = this.container.querySelector('.nav-icons-container');
        ...
        // Update background blur size
        this.updateBackgroundBlurSize();
    }

    // Update touch interactions for cross-device compatibility - Requirements 8.4, 8.5
    updateTouchInteractions(isMobile) {
        const navIcons = this.container.querySelectorAll('.nav-icon');

        navIcons.forEach(icon => {
            // Ensure minimum 44px touch targets - Requirement 8.5
            const currentMinWidth = parseInt(getComputedStyle(icon).minWidth);
            const currentMinHeight = parseInt(getComputedStyle(icon).minHeight);

            if (currentMinWidth < 44) {
                icon.style.minWidth = '44px';
            }
            ...
            if (isMobile) {
                // Enhanced touch interactions for mobile - Requirement 8.4
                icon.style.touchAction = 'manipulation';
                icon.style.webkitTouchCallout = 'none';
                icon.style.webkitUserSelect = 'none';
                icon.style.userSelect = 'none';
                ...
                this.addMobileTouchListeners(icon);
            } else {
                // Desktop-specific interactions
                this.removeMobileTouchListeners(icon);
            }
        });
    }

    // Add mobile-specific touch listeners - Requirement 8.4
    addMobileTouchListeners(icon) {
        ...
        icon._touchStartHandler = touchStartHandler;
        icon._touchEndHandler = touchEndHandler;
        icon._touchCancelHandler = touchCancelHandler;

        // Add event listeners
        icon.addEventListener('touchstart', touchStartHandler, { passive: true });
        icon.addEventListener('touchend', touchEndHandler, { passive: true });
        icon.addEventListener('touchcancel', touchCancelHandler, { passive: true });
    }

    // Remove mobile-specific touch listeners
    removeMobileTouchListeners(icon) {
        if (icon._touchStartHandler) {
            icon.removeEventListener('touchstart', icon._touchStartHandler);
            delete icon._touchStartHandler;
        }
        if (icon._touchEndHandler) {
            icon.removeEventListener('touchend', icon._touchEndHandler);
            delete icon._touchEndHandler;
        }
        if (icon._touchCancelHandler) {
            icon.removeEventListener('touchcancel', icon._touchCancelHandler);
            delete icon._touchCancelHandler;
        }
    }

    // Setup mobile-specific navigation behavior
    setupMobileNavigation() {
        // Add touch-friendly spacing and interactions
        this.navItems.forEach(item => {
            ...
            item.addEventListener('touchstart', (e) => {
                longPressTimer = setTimeout(() => {
                    // Show tooltip or additional info on long press
                    this.showMobileTooltip(item);
                }, 500);
            }, { passive: true });
        ...
        // Adjust halo size for mobile
        if (this.halo) {
            this.halo.style.width = '50px';
            this.halo.style.height = '50px';
        }
    ...
    // Remove mobile-specific navigation behavior
    removeMobileNavigation() {
        this.navItems.forEach(item => {
            item.classList.remove('mobile-nav-item');
            // Remove mobile-specific event listeners by cloning
            const newItem = item.cloneNode(true);
            item.parentNode.replaceChild(newItem, item);
        });

        // Re-initialize after removing mobile listeners
        this.navItems = Array.from(this.container.querySelectorAll('.nav-item'));
        this.bindEvents();

        // Reset halo size for desktop
        if (this.halo) {
            this.halo.style.width = '40px';
            this.halo.style.height = '40px';
        }
    }
    ...
    setupMobileGestures() {
        ...
        this.container.addEventListener('touchmove', (e) => {
            ...
            // Horizontal swipe on navigation
            if (!isScrolling && deltaX > 30) {
                // Add visual feedback for navigation swipe
        ...
        this.container.addEventListener('touchend', (e) => {
            ...
            if (!isScrolling && touchStartX && touchStartY) {
                const touchEndX = e.changedTouches[0].clientX;
                const deltaX = touchStartX - touchEndX;

                // Swipe left/right to navigate between items
                if (Math.abs(deltaX) > 50) {
                    const currentIndex = this.navItems.findIndex(item => item.classList.contains('active'));
                    let newIndex;
                    ...
                    // Simulate click on new item
                    this.handleItemClick(e, this.navItems[newIndex]);
                }
            }
    ...
    // Show mobile tooltip
    showMobileTooltip(item) {
        const label = item.getAttribute('aria-label');
        if (!label) return;

        // Create tooltip element
        const tooltip = document.createElement('div');
        tooltip.className = 'mobile-nav-tooltip';
        ...
        // Announce to screen readers
        LeonPortfolio.utils.announceToScreenReader(`Tooltip: ${label}`);
    }

    // Show tooltip for accessibility (keyboard focus)
    showTooltipForAccessibility(item) {
        // Force tooltip visibility for keyboard users
        item.classList.add('tooltip-visible');

        // Announce tooltip content to screen readers
        const tooltipText = item.getAttribute('data-label');
        if (tooltipText) {
            LeonPortfolio.utils.announceToScreenReader(`Tooltip: ${tooltipText}`, 'assertive');
        }
    }

    // Hide tooltip for accessibility
    hideTooltipForAccessibility(item) {
        // Remove forced tooltip visibility
        item.classList.remove('tooltip-visible');
    }

    // Handle orientation change - Requirements 8.1, 8.2, 8.3
    handleOrientationChange(isLandscape) {
        // Ensure header height is maintained in all orientations - Requirement 8.1
        this.ensureHeaderHeight();

        // Ensure icon height is maintained - Requirement 8.2
        this.ensureIconHeight();

        // Update layout for orientation - Requirement 8.3
        this.updateContainerLayout();

        // Adjust spacing for landscape mode on mobile
        if (LeonPortfolio.state.isMobile && isLandscape) {
            const iconsContainer = this.container.querySelector('.nav-icons-container');
            if (iconsContainer && window.innerHeight <= 500) {
                // Compact spacing for landscape mobile
                iconsContainer.style.gap = '1rem';
                iconsContainer.style.padding = '0.25rem 0.75rem';
                iconsContainer.style.maxHeight = '45px';
            }
        }

        // Update background blur for orientation
        this.updateBackgroundBlurSize();

        // Announce orientation change for accessibility
        LeonPortfolio.utils.announceToScreenReader(
            `Navigation adapted for ${isLandscape ? 'landscape' : 'portrait'} orientation`
        );
    }

    // Handle viewport size changes - Requirements 8.1, 8.2, 8.3
    handleViewportChange() {
        // Detect device type changes
        const wasMobile = LeonPortfolio.state.isMobile;
        const wasTablet = LeonPortfolio.state.isTablet;

        LeonPortfolio.detectMobile();
        LeonPortfolio.detectTablet();

        // Update navigation if device type changed
        if (wasMobile !== LeonPortfolio.state.isMobile || wasTablet !== LeonPortfolio.state.isTablet) {
            this.updateForMobile(LeonPortfolio.state.isMobile);
        }

        // Always update layout for viewport changes
        this.updateContainerLayout();

        // Update touch interactions based on device capabilities
        this.updateTouchInteractions(LeonPortfolio.state.isMobile);
    }

    // Detect device capabilities and update interactions - Requirements 8.4, 8.5
    detectDeviceCapabilities() {
        // Detect touch capability
        const hasTouch = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
        ...
        navIcons.forEach(icon => {
            if (hasTouch && !hasHover) {
                // Touch-only device
                icon.classList.add('touch-only');
                this.addMobileTouchListeners(icon);
            } else if (hasHover && hasFinePrecision) {
                // Mouse/trackpad device
                icon.classList.add('hover-capable');
                this.removeMobileTouchListeners(icon);
            } else {
                // Hybrid device
                icon.classList.add('hybrid-input');
                this.addMobileTouchListeners(icon);
            }
        });
    ...
    // Performance monitoring for responsive navigation
    monitorPerformance() {
    ...
    destroy() {
        ...
        this.navItems.forEach(item => {
            ...
            this.removeMobileTouchListeners(item);
            // Replace with clean clone
            item.replaceWith(item.cloneNode(true));
        });

        if (this.halo && this.halo.parentNode) {
            this.halo.parentNode.removeChild(this.halo);
        }
