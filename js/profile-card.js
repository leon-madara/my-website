        ...
        const statsObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    // Only animate if the content contains a number
                    // Skip text-only values like "Within 24 hrs" or "2 projects"
                    const hasNumber = /\d/.test(textContent);
                    
                    if (hasNumber) {
                        const targetValue = parseStatValue(textContent);
                        
                        // Only animate if we got a valid number (not NaN)
