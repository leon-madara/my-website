...
Each active feature folder should carry a machine-readable metadata file at:

- `docs/exec-plans/active/<feature-slug>/branch-context.json`
...
V1 uses one repo-wide registry containing branch relationships and status for the whole tree.
...
Lock the spec and keep the registry at `docs/git/branch-registry.json`.
...
- Use `branch-context.json` for feature-local branch metadata.
