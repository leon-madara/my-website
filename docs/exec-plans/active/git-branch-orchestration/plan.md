...
- [ ] Decide whether v1 should be skill-only guidance or skill plus helper script
- [ ] Confirm the canonical location for the future branch registry
- [ ] Confirm the per-feature metadata fields that active execution-plan folders must carry

### Phase 3 - Build the workflow tooling

- [ ] Create the `git-branch-orchestrator` skill
- [ ] Add a helper script to inspect branch ancestry, upstream state, and allowed promotion targets
- [ ] Define the pre-flight and post-task invocation flow for agents

### Phase 4 - Validate the workflow on real feature branches

- [ ] Test branch selection against at least two active feature branches
- [ ] Test refusal behavior for ambiguous or unsafe push or merge attempts
