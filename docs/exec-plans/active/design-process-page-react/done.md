...
- Added React route `/design-process` and a v1 stub page with the SVG mural hero.
- Added `Process` nav item between `Portfolio` and `Contact` (desktop primary nav + mobile nav).
- Implemented glass capsule mobile bottom nav styling with icon-only items and an active bubble state.
